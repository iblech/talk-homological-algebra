...
R^2F(X) \to \cdots \]
depending functorially on the given short exact sequence. This way
...
of~$\Hom_\A(T,\placeholder)$ and of~$\Hom_\A(\placeholder,T)$ follow from
...
epimorphism~$X \twoheadrightarrow Y$ and any morphism~$P \to Y$, there exists a
co-lift of that morphism to~$X$.
\[ \xymatrix{
   X \ar@{->>}[r] & Y \ar[r] & 0 \\
  & P \ar[u] \ar@{-->}[lu]
} \]
...
No uniqueness of the lifts is required.
...
it is \emph{divisible}, that is if for any~$x \in G$ and~$n \geq 1$ there exists an
...
of~$1$ under a lift of the map~$\ZZ \to G,\,1 \mapsto x$. The standard proof of
the converse direction employs the axiom of choice.} For example, the
...
its number of elements is not a multiple of~$6$. (Geometrically, it is a vector
bundle whose rank varies on the two components of~$\Spec \ZZ/(6)$.)\end{ex}
...
notes on commutative algebra} or
\href{https://rawgit.com/iblech/talk-homological-algebra/master/kaplansky-en.pdf}{this
linked one-page note}.}
It is a
...
The statement is totally false without the injectivity or
...
sequence~$P^\bullet \to X \to 0$ with the objects~$P^{-n}$,~$n \geq 0$, projective.
\end{defn}

\begin{ex}\label{ex:syzygies-resolution}Let~$M$ be an~$A$-module. Picking some
system of generators, we obtain a surjective map~$A^{(I)} \twoheadrightarrow M$.
If the chosen system is linearly independent, this map is an isomorphism and~$0
...
groups on a topological space has enough injectives, but lacks enough
...
if and only if~$\Ext^1(P,T) = 0$ for all objects~$T$.} The homological
...
homotopy equivalent. Furthermore, the witnessing homotopy equivalence is unique
up to homotopy, if we demand compatibility with the augmentations.\end{cor}
...
false.
\footnotetext{If~$K^\bullet \to L^\bullet$ is a quasi-isomorphism, there is usually no
...
Y^\bullet$. This complication is a source of technical difficulties. To say
...
cohomology. Even though the complex~$I^\bullet$ is not acyclic only
at position zero, the complex~$F(I^\bullet)$ may have cohomology in arbitrarily
high degrees~(!).

Since we made an arbitrary choice of an injective resolution, we have to
...
well-defined up to homotopy equivalence.\footnote{The isomorphism on cohomology
(respectively the isomorphism on complexes in the \emph{derived category}) is
unique with a certain property. This is what allows to speak of ``the'' derived
functor.}
...
But~$R^{n+1} F$ is \emph{not} the first derived functor of~$R^n F$
(indeed, in general the higher derived functors are neither left- nor
right-exact, so we cannot derive them) and there is no Leibniz rule to
be found.
...
that exact functors commute with cohomology. Conversely, we'll see below that a
left-exact functor for which all higher derived functors vanish is
exact.\end{ex}
...
The torsion did not vanish, but instead moved to~$R^1$.
\end{ex}
...
The lemma shows very visibly that~$F$ is exact if and only if~$R^1 F
= 0$.
...
F(X) \cong H^n(F(U^\bullet))$.\footnote{The complex~$\RR F(X)$ will in general
...
\[ \pi_*\E \otimes \F \cong \pi_*(\E \otimes \pi^*\F), \]
valid in the case that~$\F$ is a locally free sheaf.
For the derived functors, the formula holds in complete generality, assuming
only some finiteness conditions:
\[ \RR\pi_*(\E^\bullet) \stackrel{\LL}{\otimes} \F^\bullet \simeq_\text{qis}
\RR\pi_*(\E^\bullet \stackrel{\LL}{\otimes} \LL\pi^*\G^\bullet). \]
...
\[ V(\I) \ip V(\J) \defeq \chi(\O_X/\I \stackrel{\LL}{\otimes}_{\O_X} \O_X/\J) \defeq
...
In algebraic geometry, there are are important examples of schemes which ``should'' be
...
to fix these problems is to turn to \emph{(dg-)enhancements of derived categories}.
\end{rem}
...
missing zero at the front, you can hear this sequence crying ``derive
me!''.\footnote{The author learned of these cries from Timo Schürg.}
