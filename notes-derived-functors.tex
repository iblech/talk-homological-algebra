...
\begin{abstract}These notes give a self-contained exposition of the basic
...
functors. No prior knowledge of homological algebra is assumed, however, one
should be familiar with the notions of exact sequences and chain complexes, as
...
right to obtain an exact sequence. The \emph{right-derived
functors} of~$F$ answer this question: There is a long exact sequence
\[ 0 \to F(X) \to F(Y) \to F(Z) \to R^1F(X) \to R^1F(Y) \to R^1F(Z) \to
R^2F(X) \to \cdots \]
depending functorially on the given short exact sequence.
To construct~$R^n F(X)$, one picks an \emph{injective resolution}~$0 \to X \to
I^\bullet$ of~$X$ and sets~$R^n F(X) \defeq H^n(F(I^\bullet))$.
\vfill
...
\item For any morphism~$f : X \to Y$, there exists a \emph{kernel}~$K
\hookrightarrow X$ and a \emph{cokernel}~$Y \twoheadrightarrow K$.
...
morphism of such sheaves is calculated as its naive presheaf kernel, the
...
\Hom_\B(FX,FY)$ are homomorphisms of groups. An additive functor~$F$ is
...
in~$\B$. It is \emph{left-exact} if at least~$0 \to FX \to FY \to FZ$ is still
exact and \emph{right-exact} if at least~$FX \to FY \to FZ \to 0$ is
exact.\end{defn}

\begin{ex}Let~$T$ be an~$A$-module. The functor~$\Mod(A) \to \Ab,\,M
\mapsto M \otimes_A T$ is always right-exact. It is exact if and only if~$T$
...
preserves finite colimits.\end{lemma}
...
$D^b(X)$ as an invariant, derived equivalence, $f^!$
